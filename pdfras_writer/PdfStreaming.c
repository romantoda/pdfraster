...
//todo rt
// when encrypting streams we prepare temp_sink that collects data into 
// buffer and when sink free is called it encrypts buffer and flushes encrypted data into
//original sink
typedef struct t_crypt_sink_stream {
  t_datasink* out_sink;
  t_pdoutstream* stm;
  pduint8* in_buffer;
  pduint32 in_buffer_position;
} t_crypt_sink_stream;

static pdbool crypt_put(const pduint8 *buffer, pduint32 offset, pduint32 len, void *cookie)
{
  t_crypt_sink_stream* t = (t_crypt_sink_stream*)cookie;
  t->in_buffer = realloc(t->in_buffer, t->in_buffer_position + len);
  memcpy(t->in_buffer + t->in_buffer_position, buffer, len);
  t->in_buffer_position += len;
  return PD_TRUE;
}

static void crypt_free(void *cookie)
{
  t_crypt_sink_stream* t = (t_crypt_sink_stream*)cookie;

  t_pdencrypter* encrypter = t->stm->encrypter;
  t_pdmempool* pool = pd_get_pool(t->stm);

  pduint32 enc_len = pd_encrypted_size(encrypter, t->in_buffer_position);
  pduint8* enc_buffer = (pduint8*)pd_alloc(pool, enc_len);
  t_pdstring* encstr = pd_string_new_binary(pool, enc_len, NULL);
  pd_encrypt_data(encrypter, enc_buffer, t->in_buffer, t->in_buffer_position);
  pd_datasink_put(t->out_sink, enc_buffer, 0, enc_len);
  free(t->in_buffer);
}
...
static void writestreambody(t_pdoutstream *os, t_pdvalue dict)
{
  ...
  if (sink) {
    ...
    if (pd_stream_is_encrypted(os)) {
      t_pdmempool* pool = pd_get_pool(os);
      t_crypt_sink_stream t;
      t.out_sink = sink;
      t.stm = os;
      t.in_buffer = NULL;
      t.in_buffer_position = 0;
      t_datasink* temp_sink = pd_datasink_new(pool, crypt_put, crypt_free, &t);
      stream_write_data(dict, temp_sink);
      pd_datasink_free(temp_sink);
    }
    else
...
void pd_write_reference_declaration(t_pdoutstream *stm, t_pdvalue ref)
{
  if (stm && IS_REFERENCE(ref)) {
    ...
    if (!pd_reference_is_written(ref)) {
      ...
      //todo rt hack - don't encrypt /Encrypt dict
      t_pdencrypter *stored_encrypter = NULL;
      if (pd_stream_is_encrypted(stm)) {
        stored_encrypter = stm->encrypter;
        pd_encrypt_start_object(stm->encrypter, onr, 0);
        if (onr == stm->encrypter->id_encrypt)
          stm->encrypter = NULL;
      }
